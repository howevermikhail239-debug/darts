...
const snapshot = (match: Match): SessionSnapshot => {
  ...
  return {
    match,
    draft,
    evaluation: rulesFor(match).evaluateDraft(draft, match),
    isConfirming: false,
...
describe('GameViewModel', () => {
  it('maps an ordinary X01 turn and the authoritative action state', () => {
    ...
    expect(view).toMatchObject({
      title: '501 · до победы',
      phaseLabel: 'Точный выход в 0',
      currentPlayerName: 'Анна',
      currentPlayerDetail: 'остаток: 401',
  ...
  it('maps a fixed-visits tie decision', () => {
    ...
    expect(toGameViewModel(snapshot(tied), players)).toMatchObject({
      awaitingTieDecision: true,
      canCompleteDraw: true,
      phaseLabel: 'Ничья после основных подходов',
    });
  });
  ...
  it('maps X01 tie-break decision and extra round without exposing phase unions to the page', () => {
    ...
    const extra: Match = {
      ...
      state: {
        ...match.state,
        format: { kind: 'limited', visitsPerPlayer: 1 },
        phase: { kind: 'tie_break', playerIds: ['a', 'b'], completedPlayerIds: [], roundScores: {}, round: 2 },
      },
    };
    expect(toGameViewModel(snapshot(firstTie), players)).toMatchObject({
      awaitingTieDecision: true,
      canCompleteDraw: false,
      phaseLabel: 'Ничья по минимальному остатку',
    });
    expect(toGameViewModel(snapshot(awaiting), players)).toMatchObject({
      awaitingTieDecision: true,
      canCompleteDraw: true,
      phaseLabel: 'Ничья по минимальному остатку',
    });
    expect(toGameViewModel(snapshot(extra), players)).toMatchObject({
      inExtraRound: true,
      phaseLabel: 'Дополнительный подход 2',
    });
  });
  ...
  it('maps a fixed-visits extra round', () => {
    ...
    expect(toGameViewModel(snapshot(extra), players)).toMatchObject({
      inExtraRound: true,
      phaseLabel: 'Дополнительный подход 3',
    });
  });
  ...
  it('maps completed and abandoned states', () => {
    ...
    expect(toGameViewModel(snapshot(completed), players)).toMatchObject({
      completed: true,
      abandoned: false,
      summaryEyeline: '501',
      summaryTitle: 'Анна победила',
    });
