...
describe("phase persistence compatibility", () => {
  it("accepts every new and legacy phase shape", () => {
    const fixed = createMatch("f", ["a", "b", "c"], { mode: "fixed_visits", visitsPerPlayer: 1, startingPlayerIndex: 0 }, at);
    if (fixed.state.kind !== "fixed_visits") throw new Error("setup");
    const withPhase = (phase: unknown): Match => ({ ...fixed, status: "in_progress", state: { ...fixed.state, phase } as never });
    for (const phase of [
    ...
    const drawn: Match = { ...withPhase({ kind: "completed_draw", playerIds: ["a", "b"], round: 2 }), status: "completed", completedAt: at };
    expect(isStoredMatch(drawn)).toBe(true);

    const x01 = createMatch("x", ["a", "b"], { mode: "x01", format: { kind: "limited", visitsPerPlayer: 1 }, startingPlayerIndex: 0 }, at);
    if (x01.state.kind !== "x01") throw new Error("setup");
    const x01Draw: Match = { ...x01, status: "completed", completedAt: at, state: { ...x01.state, phase: { kind: "completed_draw", playerIds: ["a", "b"], round: 2 } } };
