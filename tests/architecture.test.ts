...
describe('architecture boundaries', () => {
    ...
    const violations = Object.entries(sources)
      ...
      .flatMap(([file, source]) =>
        importsOf(source)
          .filter(
            (target) =>
              /(?:^|\/)(application|infrastructure|presentation|app)(?:\/|$)/.test(target) || target === 'react',
          )
          .map((target) => `${file} -> ${target}`),
      );
  ...
  it('keeps application independent from infrastructure, presentation, React, and fetch', () => {
    ...
    const importViolations = applicationSources.flatMap(([file, source]) =>
      importsOf(source)
        .filter((target) => /(?:^|\/)(infrastructure|presentation|app)(?:\/|$)/.test(target) || target === 'react')
        .map((target) => `${file} -> ${target}`),
    );
    const fetchViolations = applicationSources
