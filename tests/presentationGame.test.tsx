...
const players = [{ id: "a", name: "Миша", createdAt: at }, { id: "b", name: "Саша", createdAt: at }];
...
class GatedRepository implements MatchRepository {
  ...
  async loadActive(): Promise<ActiveMatchRecord | undefined> { return undefined; }
  async archiveAndClearActive(): Promise<void> { /* не нужен в этом тесте */ }
  async listHistory(): Promise<readonly []> { return []; }
...
const renderGame = () => {
  ...
  const match = createMatch("m1", ["a", "b"], { mode: "x01", format: { kind: "unlimited" }, startingPlayerIndex: 0 }, at);
  const session = new GameSession(match, repository, () => "visit", () => at);
  render(
    <GamePage
      session={session}
