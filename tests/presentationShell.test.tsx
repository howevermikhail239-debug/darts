...
describe('company membership is reversible and explicit (DATA-6)', () => {
  ...
  it('asks for confirmation before leaving a company and shows the invite link as text', () => {
    ...
    render(
      <SetupPage
        ...
        inviteLink="https://darts.example/g/token-a"
      />,
    );
  ...
  it('lists the companies this device already knows and offers to return to them', () => {
    ...
    render(
      <KnownCompanies
        companies={[company, { token: 'token-b', name: 'Соседи', createdAt: '2026-08-01T10:00:00.000Z' }]}
...
describe('setup page reports failures instead of swallowing them (CLI-2)', () => {
  ...
  it('shows why a company player could not be added', async () => {
    const onAddSharedPlayer = vi
      .fn()
      .mockRejectedValue(new Error('Не удалось связаться с компанией. Проверьте подключение.'));
    render(
      <SetupPage
        saved={[]}
        onStart={vi.fn()}
    ...
    await waitFor(() =>
      expect(screen.getByRole('alert')).toHaveTextContent('Не удалось связаться с компанией. Проверьте подключение.'),
    );
...
describe('user-facing error texts (OOP-1)', () => {
  it('passes through known user conditions and hides unknown invariant violations', () => {
    ...
    expect(userMessage(new Error('Сначала сбросьте незавершённый подход'))).toBe(
      'Сначала сбросьте незавершённый подход',
    );
