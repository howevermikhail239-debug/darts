...
function visit(
  ...
  options: { awarded?: number; result?: VisitResult } = {},
): Visit {
  ...
  return {
    ...
    matchId: 'match',
    playerId,
    visitIndex: Number(id.replace(/\D/g, '')) || 0,
    darts,
    physicalDartsUsed: darts.length,
    rawScore,
    awardedScore: options.awarded ?? rawScore,
    before: { scores: { a: 501, b: 501, c: 501 }, currentPlayerIndex: 0 },
    after: { scores: { a: 501, b: 501, c: 501 }, currentPlayerIndex: 1 },
    result: options.result ?? 'scored',
    timestamp: at(1),
  };
}
function match(
  ...
  const base = createMatch(
    ...
    players,
    mode === 'x01'
      ? { mode, format: { kind: 'unlimited' }, startingPlayerIndex: 0 }
      : { mode, visitsPerPlayer: 1, startingPlayerIndex: 0 },
    at(day),
  );
  return {
    ...base,
    id,
    createdAt: at(day),
    ...(status === 'completed' ? { completedAt: at(day) } : {}),
    status,
    confirmedVisits: visits.map((item) => ({ ...item, matchId: id })),
    ...(winnerId ? { winnerId } : {}),
  };
}
...
describe('statistics projections', () => {
  ...
  it('uses cumulative thresholds, exact buckets and a transparent standard deviation', () => {
    ...
    const visits = scores.map((score, index) =>
      visit(
        `v${index}`,
        'a',
        score === 0
          ? [miss()]
          : score === 180
            ? [numberThrow(20, 3), numberThrow(20, 3), numberThrow(20, 3)]
            : [numberThrow(Math.min(20, score), 1)],
        { awarded: score },
      ),
    );
    // The explicit score is what a confirmed visit stores; physical darts remain real facts.
    const confirmed = visits.map((item, index) => ({
      ...item,
      rawScore: scores[index]!,
      awardedScore: scores[index]!,
    }));
  ...
  it('charges a bust the full three darts so that busting earlier cannot improve the average', () => {
    ...
    const early = statisticsForVisits(
      [scored, visit('v2', 'a', [numberThrow(20, 3)], { awarded: 0, result: 'bust' })],
      'a',
    );
    const late = statisticsForVisits(
      [
        scored,
        visit('v3', 'a', [numberThrow(20, 1), numberThrow(20, 1), numberThrow(20, 3)], { awarded: 0, result: 'bust' }),
      ],
      'a',
    );
  ...
  it('returns finite zeroes with no physical darts', () => {
    ...
    expect(
      [
        stats.averagePerDart,
        stats.averagePerVisit,
        stats.threeDartAverage,
        stats.resultSpread,
        ...stats.positions.flatMap((position) => [position.average, position.missPercent, position.triplePercent]),
  ...
  it('includes abandoned hits in overall statistics but excludes them from records and trends', () => {
    const abandoned = match(
      ...
      1,
      'x01',
      [visit('v1', 'a', [numberThrow(20, 3), numberThrow(20, 3), numberThrow(20, 3)])],
      undefined,
      ['a', 'b'],
      'abandoned',
    );
    ...
    expect(trendForPlayer([abandoned, completed], 'a', 'bestVisit')).toEqual([
      { matchId: 'completed', date: at(2), value: 20 },
    ]);
  ...
  it('uses only earlier completed matches as the baseline for record improvements', () => {
    ...
    expect(newRecordsForMatch(improved, [abandoned, baseline], 'a').map((record) => record.key)).toEqual(
      expect.arrayContaining(['bestVisit', 'bestThreeDartAverage', 'mostTriples']),
    );
  });
  ...
  it('filters mode and last 5/10/20 without changing chronological trend order', () => {
    const matches = Array.from({ length: 24 }, (_, index) =>
      match(
        `m${index}`,
        index + 1,
        index % 2 ? 'x01' : 'fixed_visits',
        [visit(`v${index}`, 'a', [numberThrow(1, 1)])],
        'a',
      ),
    );
    ...
    expect(trendForPlayer(matches, 'a', 'bestVisit', 'x01', 5).map((point) => point.matchId)).toEqual([
      'm15',
      'm17',
      'm19',
      'm21',
  ...
  it('calculates records and applies the named percentage minimum sample', () => {
    ...
    const enoughDarts = Array.from({ length: MIN_PERCENT_RECORD_DARTS / 3 }, (_, index) =>
      visit(`e${index}`, 'a', [miss(), numberThrow(20, 3), numberThrow(20, 3)]),
    );
  ...
  it('does not celebrate the first baseline match and detects a later real record', () => {
    ...
    expect(newRecordsForMatch(better, [first], 'a').map((record) => record.key)).toEqual(
      expect.arrayContaining(['bestVisit', 'bestThreeDartAverage', 'mostTriples', 'most100Plus']),
    );
  });

  it('counts multiplayer head-to-head without turning a third-player win into a duel win', () => {
    const shared = [
      match('a', 1, 'x01', [], 'a', ['a', 'b', 'c']),
