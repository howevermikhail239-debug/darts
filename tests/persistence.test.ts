...
const deleteRawDatabase = (): Promise<void> => new Promise((resolve, reject) => {
...
const finished = (match: Match, completedAt = "2026-09-08T12:00:00.000Z"): Match =>
  ({ ...match, status: "completed", completedAt, winnerId: "a" });

describe("IndexedDB repository", () => {
  afterEach(() => clearLocalData());
  it("persists current and previous checkpoint", async () => {
    const repo = new IndexedDbMatchRepository(), m = x01("m");
  ...
  it("rejects the obsolete X01 leg format instead of silently changing its rules", async () => {
    ...
    const obsolete = {
      ...valid,
      state: { kind: "x01", targetLegWins: 1, remaining: { a: 501, b: 501 }, legsWon: { a: 0, b: 0 }, currentLeg: 1, legStarterIndex: 0 },
    };
  ...
  it("recovers after a reload once the conflicting tab's state is re-read", async () => {
    ...
    await expect(tabA.saveActive({ current: match, draft: { playerId: "a", draft: emptyDraft() } })).rejects.toThrow("другой вкладке");
    await tabA.loadActive();
    await expect(tabA.saveActive({ current: match, draft: { playerId: "a", draft: emptyDraft() } })).resolves.toBeUndefined();
  });
  ...
  it("archives a company match into the company store instead of losing its token", async () => {
    ...
    expect((await shared.matches("token-c")).map((item) => ({ id: item.matchId, state: item.state }))).toEqual([{ id: "company-match", state: "pending" }]);
