...
const completed = (index: number): Match => ({
  ...createMatch(`m${index}`, ["a", "b"], { mode: "fixed_visits", visitsPerPlayer: 1, startingPlayerIndex: 0 }, `2026-09-${String(index + 1).padStart(2, "0")}T10:00:00.000Z`, { a: "Миша", b: "Саша" }),
  status: "completed",
  completedAt: at,
  winnerId: "a",
});
...
describe("history list (PERF-7, UI-3)", () => {
  const historyProps = { persistentPlayerIds: ["a", "b"], onBack: vi.fn(), onRematch: vi.fn(async () => undefined), onDelete: vi.fn(async () => undefined) };
  ...
  it("keeps the name from the match and explains the current profile name", () => {
    render(<HistoryPage matches={[completed(0)]} players={[{ id: "a", name: "Михаил", createdAt: at }]} {...historyProps} />);
