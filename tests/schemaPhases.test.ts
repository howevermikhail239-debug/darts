...
const acceptedFixedVisitsPhases: readonly (readonly [string, unknown])[] = [
  ...
  [
    'новая extra_round с очками раунда',
    { kind: 'extra_round', round: 2, playerIds: ['a', 'b'], roundScores: { a: 40, b: 0 }, completedPlayerIds: ['a'] },
  ],
];
const acceptedX01Phases: readonly (readonly [string, unknown])[] = [
  ...
  [
    'tie_break',
    { kind: 'tie_break', playerIds: ['a', 'b'], completedPlayerIds: ['a'], roundScores: { a: 60 }, round: 1 },
  ],
...
describe('match phase compatibility', () => {
  ...
  it('still loads a stored match whose phase carries the new optional fields', async () => {
    ...
    const stored = withPhase(fixedVisits, {
      kind: 'extra_round',
      round: 2,
      playerIds: ['a', 'b'],
      roundScores: { a: 40, b: 40 },
      completedPlayerIds: [],
    }) as Match;
  ...
  it('treats only finished matches as shareable and caps the roster at MAX_PLAYERS', () => {
    ...
    expect(isSharedMatch({ ...x01, status: 'completed', completedAt: '2026-09-01T11:00:00.000Z', winnerId: 'a' })).toBe(
      true,
    );
