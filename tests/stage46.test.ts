...
function completed(id: string, winnerId: string | undefined, names: Record<string, string> = { a: "Миша", b: "Саша" }, status: Match["status"] = "completed"): Match {
  const ids = Object.keys(names);
  const base = createMatch(id, ids, { mode: "fixed_visits", visitsPerPlayer: 1, startingPlayerIndex: 0 }, at, names);
  const visits: Visit[] = ids.map((playerId, index) => ({ id: `${id}-v${index}`, matchId: id, playerId, visitIndex: index, inputKind: "aggregate", aggregateScore: index ? 40 : 60, physicalDartsUsed: 3, rawScore: index ? 40 : 60, awardedScore: index ? 40 : 60, before: { scores: Object.fromEntries(ids.map((item) => [item, 0])), currentPlayerIndex: index }, after: { scores: Object.fromEntries(ids.map((item) => [item, 0])), currentPlayerIndex: (index + 1) % ids.length }, result: winnerId === playerId ? "match_won" : "scored", timestamp: at }));
...
describe("Stage 4.6 Last Setup", () => {
  ...
  it("preserves stable and temporary participants plus every real match option", () => {
    const template = createLastSetupTemplate([{ name: "Миша", playerId: "p1" }, { name: "Дима" }], { mode: "x01", startingScore: 701, outRule: "double", format: { kind: "limited", visitsPerPlayer: 9 }, startingPlayerIndex: 1 });
    expect(template).toEqual({ participants: [{ name: "Миша", playerId: "p1" }, { name: "Дима" }], setup: { mode: "x01", startingScore: 701, outRule: "double", format: { kind: "limited", visitsPerPlayer: 9 }, startingPlayerIndex: 1 } });
    expect(restoreLastSetupParticipants(template, [player("p1", "Михаил")])).toEqual([{ name: "Михаил", playerId: "p1" }, { name: "Дима" }]);
  });

  it("marks a missing profile by id and never substitutes a same-named player", () => {
    const template = createLastSetupTemplate([{ name: "Миша", playerId: "gone" }, { name: "Гость" }], { mode: "fixed_visits", visitsPerPlayer: 5, startingPlayerIndex: 0 });
    expect(restoreLastSetupParticipants(template, [player("replacement", "Миша")])?.[0]).toEqual({ name: "Миша", missingPlayerId: "gone" });
  });

  it("isolates local and each company by stable token", async () => {
    const repository = new IndexedDbLastSetupRepository();
    const local = createLastSetupTemplate([{ name: "Локальный 1" }, { name: "Локальный 2" }], { mode: "x01", startingScore: 301, outRule: "straight", format: { kind: "unlimited" }, startingPlayerIndex: 0 });
    const company = createLastSetupTemplate([{ name: "Клуб 1" }, { name: "Клуб 2" }], { mode: "x01", startingScore: 701, outRule: "double", format: { kind: "unlimited" }, startingPlayerIndex: 1 });
...
describe("Stage 4.6 today and player form", () => {
  it("uses only today's completed matches and excludes abandoned results from form", () => {
    const won = completed("won", "a"), lost = completed("lost", "b"), draw = completed("draw", undefined), abandoned = completed("abandoned", undefined, { a: "Миша", b: "Саша" }, "abandoned");
    expect(recentForm([abandoned, lost, draw, won], "a")).toEqual(["loss", "draw", "win"]);
    expect(summarizeToday([won, lost, draw, abandoned], ["a", "b"], new Date(at))).toMatchObject({ completedMatches: 3, leader: { wins: 1, tied: true }, bestVisit: { name: "Миша", score: 60 }, maximums: 0 });
  });
  ...
  it("returns no card for zero completed matches today and supports 3+ players", () => {
    ...
    expect(summarizeToday([three], ["a", "b", "c"], new Date(at))).toMatchObject({ completedMatches: 1, leader: { name: "В", wins: 1 } });
  });
});
...
describe("Stage 4.6 result sharing", () => {
  ...
  it("uses Web Share, falls back to download, and treats cancellation as normal", async () => {
    ...
    const share = vi.fn().mockResolvedValue(undefined), download = vi.fn();
    expect(await deliverResultFile(file, { canShare: () => true, share } as Pick<Navigator, "share" | "canShare">, download)).toBe("shared");
    ...
    expect(await deliverResultFile(file, { canShare: () => true, share } as Pick<Navigator, "share" | "canShare">, download)).toBe("cancelled");
  });
});
