...
test('companies isolate data, preserve stable players and idempotent immutable matches after restart', async () => {
  ...
  try {
    ...
    const item = match('match-1'); const put = jsonPut(item);
    ...
    const changed = await json(`${server.url}/api/groups/${a.body.token}/matches/match-1`, jsonPut({ ...item, completedAt: 'different' }));
    ...
    assert.equal((await json(`${server.url}/api/groups/${a.body.token}/matches/bad`, jsonPut({ id: 'bad' }))).status, 400);
    ...
    const writes = Array.from({ length: 12 }, (_, index) => json(`${server.url}/api/groups/${a.body.token}/matches/concurrent-${index}`, jsonPut(match(`concurrent-${index}`))));
...
test('profile management and match deletion are atomic, idempotent, and survive restart', async () => {
  ...
  try {
    ...
    const reset = await json(`${server.url}/api/groups/${token}/players/${id}/statistics-reset`, { method: 'POST', headers: { 'content-type': 'application/json' } });
...
test('runtime persistence failure returns 503 without committing phantom state', async () => {
  ...
  try {
    ...
    const failed = await json(`${server.url}/api/groups/${created.body.token}/players`, jsonPost({ name: 'Not persisted' }));
    assert.equal(failed.status, 503); assert.deepEqual(failed.body, { error: 'unavailable' });
    const health = await json(`${server.url}/healthz`); assert.equal(health.status, 503); assert.equal(health.body.status, 'unavailable'); assert.equal(typeof health.body.revision, 'string');
    // Reads stay available while the storage is degraded.
    const readable = await json(`${server.url}/api/groups/${created.body.token}`);
    assert.equal(readable.status, 200); assert.deepEqual(readable.body.players, []);
...
test('a multi-byte body split across TCP chunks round-trips unchanged', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-utf8-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    ...
    for (let index = 70 * 1024; index < bytes.length; index += 1) {
      if (bytes[index] >= 0xd0 && bytes[index] <= 0xd3) { splitAt = index + 1; break; }
    }
...
test('identical payloads never conflict regardless of TCP framing or key order', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-conflict-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    ...
    assert.equal(shuffled.status, 200); assert.equal(shuffled.body.revision, 1);
...
test('storage recovers by itself once the write failure is gone', async () => {
  const rootDir = await mkdtemp(join(tmpdir(), 'darts-recovery-')); const dataDir = join(rootDir, 'data'); const file = join(dataDir, 'store.json'); let server;
  try {
    server = await start(file, { DARTS_STORAGE_PROBE_MS: '100' });
    ...
    await rm(dataDir, { recursive: true });
    await writeFile(dataDir, 'blocks-directory-creation', 'utf8');
    assert.equal((await json(`${server.url}/api/groups/${created.body.token}/players`, jsonPost({ name: 'Ghost' }))).status, 503);
    assert.equal((await json(`${server.url}/healthz`)).status, 503);
    ...
    const player = await json(`${server.url}/api/groups/${created.body.token}/players`, jsonPost({ name: 'Восстановлен' }));
    assert.equal(player.status, 201);
...
test('quotas are refused with explicit codes', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-quota-')); const file = join(dir, 'store.json'); let server;
  try {
    ...
    const token = created.body.token;
    const visits = await json(`${server.url}/api/groups/${token}/matches/visits`, jsonPut({ ...match('visits'), confirmedVisits: Array.from({ length: 5001 }, () => 0) }));
    assert.equal(visits.status, 413); assert.equal(visits.body.error, 'too_many_visits');
    ...
    assert.equal(large.status, 413); assert.equal(large.body.error, 'match_too_large');
    ...
    assert.equal(third.status, 409); assert.equal(third.body.error, 'too_many_matches');
    ...
    assert.equal(extra.status, 409); assert.equal(extra.body.error, 'too_many_players');
    // A structurally inconsistent match is refused as well.
    const inconsistent = await json(`${server.url}/api/groups/${token}/matches/bad-winner`, jsonPut({ ...match('bad-winner'), winnerId: 'nobody' }));
    assert.equal(inconsistent.status, 400); assert.equal(inconsistent.body.error, 'invalid_match');
  } finally { if (server) await stop(server.child); await rm(dir, { recursive: true, force: true }); }
});

test('company creation is rate limited per address', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-rate-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    ...
    const other = await fetch(`${server.url}/api/groups`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-forwarded-for': '203.0.113.7, 10.0.0.1' }, body: '{}' });
    assert.equal(other.status, 201);
  } finally { if (server) await stop(server.child); await rm(dir, { recursive: true, force: true }); }
});

test('prototype keys are not addressable and never reach the disk', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-proto-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    ...
    for (const key of ['__proto__', 'constructor', 'toString', 'valueOf', 'hasOwnProperty']) {
      ...
      assert.equal((await json(`${server.url}/api/groups/${token}/players/${encoded}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'Ghost' }) })).status, 404, `PATCH ${key}`);
      assert.equal((await json(`${server.url}/api/groups/${token}/players/${encoded}/statistics-reset`, { method: 'POST', headers: { 'content-type': 'application/json' } })).status, 404, `reset ${key}`);
      assert.equal((await json(`${server.url}/api/groups/${token}/players/${encoded}`, { method: 'DELETE' })).status, 404, `DELETE player ${key}`);
      assert.equal((await json(`${server.url}/api/groups/${token}/matches/${encoded}`, jsonPut({ ...match(key), id: key }))).status, 404, `PUT ${key}`);
      assert.equal((await json(`${server.url}/api/groups/${token}/matches/${encoded}`, { method: 'DELETE' })).status, 404, `DELETE match ${key}`);
    }
    const snapshot = await json(`${server.url}/api/groups/${token}`);
    assert.deepEqual(snapshot.body.players, []); assert.deepEqual(snapshot.body.matches, []);
    const stored = JSON.parse(await readFile(file, 'utf8'));
    const group = Object.values(stored.groups)[0];
    assert.deepEqual(Object.keys(group.players), []); assert.deepEqual(Object.keys(group.matches), []);
...
test('SIGTERM drains the in-flight request and leaves no temporary files', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-shutdown-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    ...
    await once(socket, 'connect');
    socket.write(`POST /api/groups HTTP/1.1\r\nHost: 127.0.0.1\r\nConnection: close\r\nContent-Type: application/json\r\nContent-Length: ${payload.length}\r\n\r\n`);
...
test('security headers are present on API and static responses', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-headers-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    const expected = {
...
test('bodies must be declared as JSON', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-content-type-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    const plain = await json(`${server.url}/api/groups`, { method: 'POST', headers: { 'content-type': 'text/plain' }, body: '{}' });
    assert.equal(plain.status, 415); assert.equal(plain.body.error, 'unsupported_media_type');
    assert.equal((await fetch(`${server.url}/api/groups`, { method: 'POST', body: '{}' })).status, 415);
    const charset = await json(`${server.url}/api/groups`, { method: 'POST', headers: { 'content-type': 'application/json; charset=utf-8' }, body: '{}' });
    assert.equal(charset.status, 201);
  } finally { if (server) await stop(server.child); await rm(dir, { recursive: true, force: true }); }
});

test('an oversized body does not poison the following requests', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'darts-oversized-')); const file = join(dir, 'store.json'); let server;
  try {
    server = await start(file);
    const created = await json(`${server.url}/api/groups`, jsonPost({ name: 'Keepalive' }));
    for (let attempt = 0; attempt < 5; attempt += 1) {
      const oversized = await fetch(`${server.url}/api/groups/${created.body.token}/matches/big`, jsonPut({ ...match('big'), participantNames: { 'player-a': 'Я'.repeat(400_000), 'temporary-b': 'Б' } }));
    ...
    const raw = await rawRequest(server.port, { path: `/api/groups/${created.body.token}/matches/big`, body: JSON.stringify({ ...match('big'), participantNames: { 'player-a': 'Я'.repeat(400_000), 'temporary-b': 'Б' } }) });
