...
const finished = (id: string, createdAt: string): Match => {
  const base = createMatch(id, ["a", "b"], { mode: "x01", format: { kind: "unlimited" }, startingPlayerIndex: 0 }, createdAt);
  return { ...base, status: "completed", completedAt: createdAt, winnerId: "a" };
};
const lastSetup = {
  participants: [{ name: "Анна" }, { name: "Борис" }],
  setup: { mode: "x01", startingScore: 501, outRule: "straight", format: { kind: "unlimited" }, startingPlayerIndex: 0 },
};
...
async function seedVersionOne(): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    meta.put({ schemaVersion: 3, current: createMatch("active", ["a", "b"], { mode: "x01", format: { kind: "unlimited" }, startingPlayerIndex: 0 }, "2026-09-03T10:00:00.000Z"), draft: { playerId: "a", draft: emptyDraft() }, companyToken: token }, "activeMatch");
    ...
    meta.put({ token, match: finished("company-pending", "2026-09-04T10:00:00.000Z"), state: "pending" }, `match:${token}:company-pending`);
    meta.put({ token, match: finished("company-synced", "2026-09-05T10:00:00.000Z"), state: "synced" }, `match:${token}:company-synced`);
...
describe("IndexedDB schema v1 -> v2 migration", () => {
  ...
  it("moves every company record out of the meta dump and keeps the rest untouched", async () => {
    ...
    expect((await shared.matches(token)).map((item) => ({ id: item.matchId, state: item.state })).sort((a, b) => a.id.localeCompare(b.id)))
      .toEqual([{ id: "company-pending", state: "pending" }, { id: "company-synced", state: "synced" }]);
    ...
    expect(await new IndexedDbLastSetupRepository().load("local")).toMatchObject({ participants: [{ name: "Анна" }, { name: "Борис" }] });
