...
const finished = (id: string): Match => ({ ...x01(id), status: "completed", completedAt: "2026-09-01T11:00:00.000Z", winnerId: "a" });
const template = {
  participants: [{ name: "Анна" }, { name: "Борис" }],
  setup: { mode: "x01" as const, startingScore: 501 as const, outRule: "straight" as const, format: { kind: "unlimited" as const }, startingPlayerIndex: 0 },
};
...
async function seed(): Promise<void> {
  ...
  await matches.saveActive({ current: x01("active"), draft: { playerId: "a", draft: emptyDraft() }, companyToken: token });
  await matches.archiveAndClearActive(finished("archived"));
  await matches.saveActive({ current: x01("still-active"), draft: { playerId: "a", draft: emptyDraft() }, companyToken: token });
}
...
describe("backup with company data", () => {
  ...
  it("exports companies, their players, the offline queue, lastSetup and the active companyToken", async () => {
    ...
    expect((envelope.data.companyMatches as { matchId: string; state: string }[]).map((row) => row.matchId).sort())
  ...
  it("rejects an unknown backup version", async () => {
    const future = JSON.stringify({ type: BACKUP_TYPE, version: BACKUP_VERSION + 1, exportedAt: now(), data: { players: [], matches: [], settings: {} } });
