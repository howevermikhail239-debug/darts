...
const x01 = (id: string) =>
  createMatch(id, ["a", "b"], { mode: "x01", format: { kind: "unlimited" }, startingPlayerIndex: 0 }, "2026-09-01T10:00:00.000Z");
...
describe("cross-tab notifications", () => {
  ...
  it("broadcasts saves and clears of the active match to other tabs", async () => {
    ...
    await flush();
    await repository.archiveAndClearActive({ ...match, status: "completed", completedAt: "2026-09-01T11:00:00.000Z", winnerId: "a" });
    await flush();
    unsubscribe();
