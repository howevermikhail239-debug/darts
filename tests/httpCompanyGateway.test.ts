...
const base = createMatch("remote", ["a", "b"], { mode: "x01", format: { kind: "unlimited" }, startingPlayerIndex: 0 }, "2026-09-11T12:00:00.000Z");
...
const response = (matches: readonly unknown[], players: readonly unknown[] = []) =>
  ({ group: { name: "Лига", createdAt: "2026-09-11T12:00:00.000Z" }, players, matches });
...
describe("HttpCompanyGateway remote match validation", () => {
  it("accepts a complete match payload", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => json(response([valid]))));
    await expect(new HttpCompanyGateway().loadCompany("token")).resolves.toMatchObject({ matches: [{ id: "remote" }], skippedMatches: 0 });
  });

  it.each(corruptions)("skips %s instead of bricking the whole company", async (_label, corrupt) => {
    vi.stubGlobal("fetch", vi.fn(async () => json(response([corrupt(valid), { ...valid, id: "good" }]))));
  ...
  it("migrates a legacy match created before startingScore and outRule existed", async () => {
    const legacyState = Object.fromEntries(Object.entries(valid.state).filter(([key]) => key !== "startingScore" && key !== "outRule"));
    vi.stubGlobal("fetch", vi.fn(async () => json(response([{ ...valid, state: legacyState }]))));
    const snapshot = await new HttpCompanyGateway().loadCompany("token");
  ...
  it("skips a broken player without rejecting the snapshot", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async () => json(response([], [player, { id: "", name: 42 }]))));
    const snapshot = await new HttpCompanyGateway().loadCompany("token");
  ...
  it("rejects a structurally invalid envelope", async () => {
    vi.stubGlobal("fetch", vi.fn(async () => json({ group: { name: "Лига", createdAt: "x" }, players: [], matches: "nope" })));
