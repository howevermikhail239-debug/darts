...
import { bull, miss, numberThrow, outerBull, scoreOf } from '../src/domain/darts/DartThrow';
import {
  addDraftThrow,
  emptyDraft,
  removeDraftThrow,
...
const xmatch = (remaining = 501): Match => {
  const m = createMatch('m', ['a', 'b'], { mode: 'x01', format: { kind: 'unlimited' }, startingPlayerIndex: 0 }, now);
  if (m.state.kind !== 'x01') throw new Error('test setup');
  return { ...m, state: { ...m.state, remaining: { a: remaining, b: 501 } } };
};
const draft = (...darts: ReturnType<typeof miss>[]) => darts.reduce(addDraftThrow, emptyDraft());
describe('DartThrow', () => {
  ...
  it('rejects invalid sectors', () => expect(() => numberThrow(21, 3)).toThrow());
...
describe('VisitDraft', () => {
  it('adds, replaces, removes and resets immutably', () => {
    ...
    expect(replaceDraftThrow(three, 0, outerBull()).darts[0]?.kind).toBe('outer_bull');
    expect(removeDraftThrow(three).darts).toHaveLength(2);
    expect(resetDraft().darts).toHaveLength(0);
  });
  it('MISS occupies a physical slot', () => expect(addDraftThrow(emptyDraft(), miss()).darts).toHaveLength(1));
});
describe('X01Rules', () => {
  const rules = new X01Rules();
  it('evaluates normal score and 180', () => {
    expect(
      rules.evaluateDraft(draft(numberThrow(20, 3), numberThrow(20, 3), numberThrow(20, 3)), xmatch()),
    ).toMatchObject({
  ...
  it.each([[14, [numberThrow(20, 3)], 'bust']] as const)('detects bust from %s', (remaining, darts, status) =>
    expect(rules.evaluateDraft(draft(...darts), xmatch(remaining))).toMatchObject({
      status,
      awardedScore: 0,
  ...
  it('allows straight-out with singles, triples, and Bull', () => {
    ...
    expect(rules.evaluateDraft(draft(numberThrow(20, 1), numberThrow(20, 1)), xmatch(40))).toMatchObject({
      status: 'match_won',
      remainingAfter: 0,
    });
    expect(rules.evaluateDraft(draft(numberThrow(20, 3)), xmatch(60))).toMatchObject({
      status: 'match_won',
      remainingAfter: 0,
      physicalDartsUsed: 1,
    });
    expect(rules.evaluateDraft(draft(numberThrow(1, 1)), xmatch(1))).toMatchObject({
      status: 'match_won',
      remainingAfter: 0,
    });
  ...
  it('allows remaining 1 and only busts below zero', () => {
    expect(rules.evaluateDraft(draft(numberThrow(13, 1)), xmatch(14))).toMatchObject({
      status: 'in_progress',
      remainingAfter: 1,
      awardedScore: 13,
    });
    expect(rules.evaluateDraft(draft(numberThrow(13, 1), numberThrow(1, 1)), xmatch(14))).toMatchObject({
      status: 'match_won',
      remainingAfter: 0,
    });
  });
  it.each([5, 10, 20, 30, 37])('accepts a limited 501 format with %i visits', (visitsPerPlayer) =>
    expect(
      createMatch(
        'limited',
        ['a', 'b'],
        { mode: 'x01', format: { kind: 'limited', visitsPerPlayer }, startingPlayerIndex: 0 },
        now,
      ).state,
...
describe('FixedVisitsRules', () => {
  it('requires exactly three darts and sums misses', () => {
    const rules = new FixedVisitsRules(),
      m = createMatch('m', ['a', 'b'], { mode: 'fixed_visits', visitsPerPlayer: 5, startingPlayerIndex: 0 }, now);
    expect(rules.evaluateDraft(draft(numberThrow(20, 3), miss()), m).status).toBe('in_progress');
    expect(rules.evaluateDraft(draft(numberThrow(20, 3), miss(), numberThrow(20, 1)), m)).toMatchObject({
      status: 'ready_to_confirm',
      rawScore: 80,
    });
  });
  it.each([5, 10, 20, 30, 37])('accepts %i visits', (n) =>
    expect(
      createMatch('m', ['a', 'b'], { mode: 'fixed_visits', visitsPerPlayer: n, startingPlayerIndex: 0 }, now).state,
