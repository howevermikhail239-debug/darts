...
import {
  PLAYER_PALETTE,
  persistentPlayerPaletteIndex,
  playerMonogram,
  playerVisual,
} from '../src/presentation/players/playerVisuals';
...
const completed = (count: number, starter = 0): Match => ({
  ...createMatch(
    ...
    { mode: 'fixed_visits', visitsPerPlayer: 3, startingPlayerIndex: starter },
    at,
  ),
  status: 'completed',
  completedAt: at,
...
describe('Stage 4.5 player visuals', () => {
  ...
  it('keeps persistent colors stable independently of match position', () => {
    ...
    expect(
      new Set(['persistent-misha', 'persistent-sasha', 'persistent-zhenya'].map(persistentPlayerPaletteIndex)).size,
    ).toBeGreaterThan(1);
  });
...
describe('Stage 4.5 rematch configuration', () => {
  it.each([2, 3, 5, 8])('rotates the starting player for %i participants', (count) => {
    ...
    expect(
      prepareRematch(
        source,
        source.players.map((id) => player(id)),
      ).setup.startingPlayerIndex,
    ).toBe(0);
  });
  ...
  it('preserves mode/options and stable local/shared identities while recreating temporary participants', () => {
    const source: Match = {
      ...createMatch(
        ...
        at,
        { local: 'Миша', shared: 'Саша', 'temp-old': 'Игрок 3' },
      ),
      status: 'completed',
      completedAt: at,
      winnerId: 'local',
    ...
    expect(request.participants).toEqual([
      { name: 'Миша', playerId: 'local' },
      { name: 'Саша', playerId: 'shared' },
      { name: 'Игрок 3' },
    ]);
  });
});
...
describe('Stage 4.5 summary and haptics', () => {
  ...
  it('maps a tie and a temporary winner as match-local identity', () => {
    ...
    const tie: Match = {
      ...
      startingPlayerIndex: source.startingPlayerIndex,
      currentPlayerIndex: source.currentPlayerIndex,
      confirmedVisits: source.confirmedVisits,
      state: source.state,
    };
