...
const withRemaining = (m: import('../src/domain/match/models').Match, a: number) => {
...
describe('GameSession', () => {
  ...
  it('draft cannot mutate match and confirmation is atomic', async () => {
    const repo = new MemoryRepo(),
      m = createMatch('m', ['a', 'b'], { mode: 'x01', format: { kind: 'unlimited' }, startingPlayerIndex: 0 }, clock());
  ...
  it('undo fully restores bust and survives reload', async () => {
    ...
    const reloaded = new GameSession(persisted!.current, repo, id, clock, persisted!.previous, persisted!.draft);
    await reloaded.undo();
  ...
  it('fixed series gives equal visits then handles tie and extra round', async () => {
    const repo = new MemoryRepo(),
      m = createMatch('m', ['a', 'b'], { mode: 'fixed_visits', visitsPerPlayer: 1, startingPlayerIndex: 0 }, clock());
    const s = new GameSession(m, repo, id, clock);
    for (let p = 0; p < 2; p++) {
      await s.record(miss());
      await s.record(miss());
  ...
  it.each([2, 3, 4])('completes repeated Fixed Visits extra rounds across %i players', async (count) => {
    ...
    const match = createMatch(
      'extra-rounds',
      ids,
      { mode: 'fixed_visits', visitsPerPlayer: 1, startingPlayerIndex: count - 1 },
      clock(),
    );
    const session = new GameSession(match, repo, id, clock);
    const visit = async (score: boolean) => {
      await session.record(score ? numberThrow(20, 3) : miss());
      await session.record(miss());
      await session.record(miss());
      await session.confirm();
    };
  ...
  it.each([2, 3, 5, 8])('cycles turns across %i players', async (count) => {
    ...
    expect(session.snapshot().match.state).toMatchObject({
      regulationCompleted: Object.fromEntries(ids.map((playerId) => [playerId, 1])),
      phase: { kind: 'awaiting_tie_decision', round: 1 },
    });
  });
  it.each([2, 3, 5])('limited 501 waits for the full final round across %i players', async (count) => {
    ...
    for (let index = 0; index < count; index += 1) {
      await session.record(index === 0 ? numberThrow(20, 1) : miss());
      await session.record(miss());
      await session.record(miss());
      await session.confirm();
  ...
  it("keeps a 0:0 limited-501 tie break resolvable by comparing that round's scores", async () => {
    ...
    for (const points of [20, 20]) {
      await session.record(numberThrow(points, 1));
      await session.record(miss());
      await session.record(miss());
      await session.confirm();
    }
    expect(session.snapshot().match.state).toMatchObject({
      phase: { kind: 'awaiting_tie_break', round: 2, playerIds: ['a', 'b'] },
    });
    await session.extraRound();
    await session.record(numberThrow(20, 1));
    await session.record(miss());
  ...
  it('fails fast on a missing X01 remaining score and never persists NaN', async () => {
    ...
    const corrupt = {
      ...base,
      state: { ...base.state, remaining: { b: 501 } },
    } as import('../src/domain/match/models').Match;
  ...
  it('restores and confirms early finishes and busts', async () => {
    ...
    const restoredFinish = new GameSession(
      finishActive.current,
      finishRepo,
      id,
      clock,
      finishActive.previous,
      finishActive.draft,
    );
    ...
    const restoredBust = new GameSession(
      bustActive.current,
      bustRepo,
      id,
      clock,
    ...
    expect(restoredBust.snapshot().match).toMatchObject({
      currentPlayerIndex: 1,
      state: { remaining: { a: 32, b: 501 } },
    });
  });
