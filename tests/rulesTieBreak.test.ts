...
class MemoryRepo implements MatchRepository {
  ...
  async archiveAndClearActive(match: Match) {
...
const visitOf = async (game: GameSession, first = miss()) => {
  await game.record(first);
  await game.record(miss());
  await game.record(miss());
  await game.confirm();
};
...
describe('Fixed visits extra round (DOM-1)', () => {
  it('keeps a trailing player out of the extra round and out of the win', async () => {
    ...
    expect(game.snapshot().match).toMatchObject({
      currentPlayerIndex: 0,
      state: { phase: { kind: 'extra_round', playerIds: ['a', 'b'], completedPlayerIds: [], roundScores: {} } },
    });
    ...
    const applied = rules.applyConfirmedVisit(
      {
        ...
        rawScore: 3,
        awardedScore: 3,
        before: { scores: { a: 60, b: 60, c: 200 }, currentPlayerIndex: 1 },
        after: { scores: {}, currentPlayerIndex: 0 },
        result: 'scored',
        timestamp: clock(),
        darts: [],
      },
  ...
  it('narrows the next round to the leaders of the previous one', async () => {
    ...
    expect(game.snapshot().match.state).toMatchObject({
      phase: { kind: 'awaiting_tie_decision', playerIds: ['a', 'b', 'c'], round: 1 },
    });
    await game.extraRound();
    ...
    expect(game.snapshot().match.state).toMatchObject({
      phase: { kind: 'awaiting_tie_decision', playerIds: ['a', 'b'], round: 2 },
    });
    await game.extraRound();
    expect(game.snapshot().match.state).toMatchObject({
      phase: { kind: 'extra_round', playerIds: ['a', 'b'], round: 2 },
    });
  ...
  it('reads a legacy phase without playerIds as everybody plays', () => {
    ...
    expect(rules.startExtraRound(legacy).state).toMatchObject({
      phase: { kind: 'extra_round', playerIds: ['a', 'b', 'c'], round: 1 },
    });
  });
...
describe('X01 draw (DOM-2)', () => {
  ...
  it('offers the draw after an all-zero tie-break round instead of looping silently', async () => {
    ...
    await visitOf(game);
    expect(game.snapshot().match.state).toMatchObject({
      phase: { kind: 'awaiting_tie_break', playerIds: ['a', 'b'], round: 2 },
    });
...
describe('Limited X01 finishes the round (DOM-4)', () => {
  it('plays the current round out and stops there, even with visits left in the format', async () => {
    const base = createMatch(
      'mid-round',
      ['a', 'b', 'c'],
      { mode: 'x01', format: { kind: 'limited', visitsPerPlayer: 3 }, startingPlayerIndex: 0 },
      clock(),
    );
    if (base.state.kind !== 'x01') throw new Error('test setup');
...
describe('Aggregate input (DOM-5)', () => {
  ...
  it('records a real bust from a sum without inventing sectors', () => {
    expect(rules.evaluateDraft(sum(60), near(20))).toMatchObject({
      status: 'bust',
      physicalDartsUsed: 3,
      rawScore: 60,
      awardedScore: 0,
    ...
    expect(rules.evaluateDraft(sum(21), near(20, 'double'))).toMatchObject({
      status: 'bust',
      awardedScore: 0,
      remainingAfter: 20,
    });
  ...
  it('still refuses to finish the match by a sum', () => {
    expect(rules.evaluateDraft(sum(60), near(60))).toMatchObject({
      status: 'invalid',
      reason: 'Для завершения используйте ввод по дротикам',
    });
    expect(rules.evaluateDraft(sum(20), near(20, 'double'))).toMatchObject({
      status: 'invalid',
      reason: 'Для завершения используйте ввод по дротикам',
    });
    expect(rules.evaluateDraft(sum(19), near(20, 'double'))).toMatchObject({
      status: 'invalid',
      reason: 'Для завершения используйте ввод по дротикам',
    });
  });
  ...
  it('applies an aggregate bust as a lost turn', async () => {
    ...
    expect(match.confirmedVisits[0]).toMatchObject({
      result: 'bust',
      awardedScore: 0,
      rawScore: 60,
      physicalDartsUsed: 3,
...
describe('Match roster limits (DOM-7)', () => {
  ...
  it('rejects fewer than two and more than eight participants', () => {
    expect(() =>
      createMatch('few', roster(1), { mode: 'fixed_visits', visitsPerPlayer: 1, startingPlayerIndex: 0 }, clock()),
    ).toThrow('как минимум 2');
    expect(() =>
      createMatch(
        'many',
