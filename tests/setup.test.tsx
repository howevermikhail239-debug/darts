...
describe('SetupPage participants', () => {
  ...
  it('starts immediately with temporary default players and allows duplicate display names', async () => {
    ...
    await waitFor(() =>
      expect(onStart).toHaveBeenCalledWith(
        [{ name: 'Миша' }, { name: 'Миша' }],
        expect.objectContaining({ mode: 'x01' }),
      ),
    );
  });
  ...
  it('selects a saved player by stable id instead of matching by name', async () => {
    ...
    render(
      <SetupPage
        saved={[{ id: 'saved-id', name: 'Миша', createdAt: '2026-01-01' }]}
        onStart={onStart}
        onHistory={() => undefined}
    ...
    await waitFor(() =>
      expect(onStart).toHaveBeenCalledWith(
        [{ name: 'Миша', playerId: 'saved-id' }, { name: 'Игрок 2' }],
        expect.anything(),
      ),
    );
  });
  ...
  it('protects company creation from duplicate submits and keeps the name for a retry', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(
      'Не удалось создать компанию. Проверьте подключение и попробуйте ещё раз.',
    );
