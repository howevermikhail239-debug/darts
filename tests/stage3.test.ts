...
function near(score: number, outRule: 'straight' | 'double' = 'straight'): Match {
  const match = createMatch(
    'm',
    ['a', 'b'],
    { mode: 'x01', startingScore: 501, outRule, format: { kind: 'unlimited' }, startingPlayerIndex: 0 },
    now,
  );
...
class MemoryRepo implements MatchRepository {
  ...
  async saveActive(record: ActiveMatchRecord) {
    this.active = structuredClone(record);
  }
  async loadActive() {
    return this.active;
  }
  async archiveAndClearActive() {
...
describe('Stage 3 X01', () => {
  ...
  it.each([301, 501, 701] as const)('starts X01 at %i', (startingScore) => {
    const match = createMatch(
      'x',
      ['a', 'b'],
      { mode: 'x01', startingScore, outRule: 'straight', format: { kind: 'unlimited' }, startingPlayerIndex: 0 },
      now,
    );
    expect(match.state).toMatchObject({
      ...
      startingScore,
      outRule: 'straight',
      remaining: { a: startingScore, b: startingScore },
    });
  });
  it.each([301, 501, 701] as const)('supports double-out with starting score %i', (startingScore) => {
    const match = createMatch(
      'x',
      ['a', 'b'],
      { mode: 'x01', startingScore, outRule: 'double', format: { kind: 'unlimited' }, startingPlayerIndex: 0 },
      now,
    );
  ...
  it('accepts doubles and Bull, rejects outer Bull and a single to zero', () => {
    ...
    expect(rules.evaluateDraft(detailed(outerBull(), outerBull()), near(50, 'double'))).toMatchObject({
      status: 'bust',
      awardedScore: 0,
    });
    expect(rules.evaluateDraft(detailed(numberThrow(20, 1), numberThrow(20, 1)), near(40, 'double'))).toMatchObject({
      status: 'bust',
      awardedScore: 0,
    });
  });
  it('supports S1 + D1 from 3 and busts on remaining 1 or below zero', () => {
    expect(rules.evaluateDraft(detailed(numberThrow(1, 1), numberThrow(1, 2)), near(3, 'double')).status).toBe(
      'match_won',
    );
...
describe('aggregate visit input', () => {
  it('persists, restores and confirms a full aggregate visit without inventing hits', async () => {
    ...
    const match = createMatch(
      'agg',
      ['a', 'b'],
      { mode: 'x01', startingScore: 301, outRule: 'straight', format: { kind: 'unlimited' }, startingPlayerIndex: 0 },
      now,
    );
    const session = new GameSession(
      match,
    ...
    await session.setInputMode('aggregate');
    await session.setAggregateScore(100);
  ...
  it.each([-1, 181, 12.5, 179])('rejects invalid or unreachable aggregate score %s', async (score) => {
    ...
    await session.setInputMode('aggregate');
    await session.setAggregateScore(score);
...
describe('backup', () => {
  ...
  it('roundtrips profiles, history, active draft and checkpoint', async () => {
    const matches = new IndexedDbMatchRepository(),
      players = new IndexedDbPlayerRepository(),
      backups = new IndexedDbBackupRepository();
    const active = createMatch(
      'active',
      ['a', 'b'],
      { mode: 'x01', startingScore: 701, outRule: 'double', format: { kind: 'unlimited' }, startingPlayerIndex: 0 },
      now,
    );
    const history = {
      ...createMatch(
    ...
    await matches.saveActive({
      current: active,
      previous: active,
      draft: { playerId: 'a', draft: { kind: 'aggregate', score: 100, darts: [] } },
    });
    ...
    expect(await new IndexedDbMatchRepository().loadActive()).toMatchObject({
      current: { id: 'active', state: { startingScore: 701, outRule: 'double' } },
      previous: { id: 'active' },
      draft: { draft: { kind: 'aggregate', score: 100 } },
    });
    expect(await new IndexedDbMatchRepository().listHistory()).toMatchObject([
      { id: 'history', participantNames: { 'temp-a': 'Игрок 1', 'temp-b': 'Игрок 2' } },
    ]);
    expect(await new LocalSettingsRepository().load()).toEqual({ inputMode: 'aggregate' });
  });
  it.each(['not json', '{}', '{"type":"darts-scorekeeper-backup","version":99,"data":{}}'])(
    'rejects invalid input without changing current data: %s',
    async (json) => {
      const matches = new IndexedDbMatchRepository();
      const active = createMatch(
        'keep',
