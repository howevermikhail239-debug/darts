...
export type DetailedVisit = VisitBase &
  Readonly<{
    inputKind?: 'detailed';
    darts: readonly DartThrow[];
  }>;
export type AggregateVisit = VisitBase &
  Readonly<{
    inputKind: 'aggregate';
...
export type X01Format = Readonly<{ kind: 'unlimited' }> | Readonly<{ kind: 'limited'; visitsPerPlayer: number }>;
...
export type FixedVisitsState = Readonly<{
  ...
  regulationCompleted: Readonly<Record<PlayerId, number>>;
  extraRoundsCompleted: number;
...
export type Match = Readonly<{
  ...
  state: ModeState;
  confirmedVisits: readonly Visit[];
  winnerId?: PlayerId;
}>;
...
export const scoresOf = (match: Match): Readonly<Record<PlayerId, number>> =>
  match.state.kind === 'x01' ? match.state.remaining : match.state.totals;
