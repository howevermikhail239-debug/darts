...
export const draftIsEmpty = (draft: VisitDraft): boolean =>
  isDetailedDraft(draft) ? draft.darts.length === 0 : draft.score === undefined;
export const emptyDraft = (): DetailedVisitDraft => Object.freeze({ kind: 'detailed', darts: Object.freeze([]) });
export const emptyAggregateDraft = (): AggregateVisitDraft =>
  Object.freeze({ kind: 'aggregate', darts: Object.freeze([]) });
const freeze = (darts: readonly DartThrow[]): DetailedVisitDraft =>
  Object.freeze({ kind: 'detailed', darts: Object.freeze([...darts]) });
...
export const resetDraft = (kind: 'detailed' | 'aggregate' = 'detailed'): VisitDraft =>
  kind === 'aggregate' ? emptyAggregateDraft() : emptyDraft();
export const truncateDraft = (draft: VisitDraft, length: number): VisitDraft =>
  isDetailedDraft(draft) ? freeze(draft.darts.slice(0, length)) : draft;
