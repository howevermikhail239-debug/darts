...
export const isNumberSegment = (value: number): value is NumberSegment =>
...
export function numberThrow(segment: number, multiplier: Multiplier): DartThrow {
  ...
  if (![1, 2, 3].includes(multiplier)) throw new RangeError('Множитель должен быть 1, 2 или 3');
...
export const scoreOf = (dart: DartThrow): number =>
  dart.kind === 'number'
    ? dart.segment * dart.multiplier
    : dart.kind === 'outer_bull'
      ? 25
      : dart.kind === 'bull'
        ? 50
        : 0;
export const notationOf = (dart: DartThrow): string =>
  dart.kind === 'number'
    ? `${dart.multiplier === 1 ? 'S' : dart.multiplier === 2 ? 'D' : 'T'}${dart.segment}`
    : dart.kind === 'outer_bull'
      ? '25'
      : dart.kind === 'bull'
        ? 'Bull'
        : 'MISS';
export const isDouble = (dart: DartThrow): boolean =>
  dart.kind === 'bull' || (dart.kind === 'number' && dart.multiplier === 2);
