...
export const distributionKeys: readonly DistributionKey[] = [
  '0–19',
  '20–39',
  '40–59',
  '60–79',
...
const emptyPosition = (): DartPositionStatistics => ({
  physicalDarts: 0,
  points: 0,
  average: 0,
  misses: 0,
...
export function statisticsForVisits(visits: readonly Visit[], playerId: PlayerId): PlayerStatistics {
  ...
  const hitCounts: Record<string, number> = {},
    winningHits: Record<string, number> = {};
  ...
  for (const visit of own) {
    ...
    if (isDetailedVisit(visit))
      visit.darts.forEach((dart, index) => {
        knownHitDarts += 1;
        const label = notationOf(dart),
          points = scoreOf(dart),
        ...
        if (position)
          positions[index] = {
            ...position,
            physicalDarts: position.physicalDarts + 1,
            points: position.points + points,
            misses: position.misses + (dart.kind === 'miss' ? 1 : 0),
            triples: position.triples + (dart.kind === 'number' && dart.multiplier === 3 ? 1 : 0),
          };
    ...
    if (visit.result === 'match_won') {
      ...
      if (last) {
        const label = notationOf(last);
        winningHits[label] = (winningHits[label] ?? 0) + 1;
      }
      finishes.push(visit.awardedScore);
    }
  }

  const completedPositions = positions.map((position) => ({
    ...position,
    average: safeRatio(position.points, position.physicalDarts),
    missPercent: percentage(position.misses, position.physicalDarts),
    triplePercent: percentage(position.triples, position.physicalDarts),
  })) as [DartPositionStatistics, DartPositionStatistics, DartPositionStatistics];
  return {
    physicalDarts,
    knownHitDarts,
    visits: own.length,
    awardedPoints,
    rawPoints,
    averagePerDart: safeRatio(awardedPoints, scoringDarts),
    averagePerVisit: safeRatio(awardedPoints, own.length),
    threeDartAverage: safeRatio(awardedPoints * 3, scoringDarts),
    bestVisit,
    misses,
    outerBulls,
    bulls,
    ...
    triples,
    hitCounts,
    positionScores: [completedPositions[0].points, completedPositions[1].points, completedPositions[2].points],
    positions: completedPositions,
    thresholds,
    distribution,
    resultSpread: standardDeviation(scoringValues),
    winningHits,
    finishes,
  };
}

export const statisticsForMatch = (match: Match): Readonly<Record<PlayerId, PlayerStatistics>> =>
  Object.fromEntries(match.players.map((id) => [id, statisticsForVisits(match.confirmedVisits, id)]));
