...
const matchesForPlayerAndMode = (matches: readonly Match[], playerId: PlayerId, mode: StatisticsMode): readonly Match[] => chronological(matches).filter((match) => match.players.includes(playerId) && (mode === "all" || match.state.kind === mode));
...
export function filteredMatches(matches: readonly Match[], playerId: PlayerId, mode: StatisticsMode, period: StatisticsPeriod): readonly Match[] {
  return applyPeriod(matchesForPlayerAndMode(matches, playerId, mode), period);
}

function completedMatches(matches: readonly Match[], playerId: PlayerId, mode: StatisticsMode, period: StatisticsPeriod): readonly Match[] {
  return applyPeriod(matchesForPlayerAndMode(matches, playerId, mode).filter((match) => match.status === "completed"), period);
}
...
export function statisticsForPlayerHistory(matches: readonly Match[], playerId: PlayerId, mode: StatisticsMode = "all", period: StatisticsPeriod = "all"): PlayerHistoryStatistics {
  const selected = filteredMatches(matches, playerId, mode, period);
...
export function recordsForPlayer(matches: readonly Match[], playerId: PlayerId, mode: StatisticsMode = "all"): PlayerRecords {
  const perMatch = completedMatches(matches, playerId, mode, "all").map((match) => statisticsForVisits(match.confirmedVisits, playerId));
...
export function newRecordsForMatch(current: Match, previousMatches: readonly Match[], playerId: PlayerId): readonly RecordImprovement[] {
  ...
  const prior = previousMatches.filter((match) => match.id !== current.id && match.status === "completed" && match.players.includes(playerId));
  ...
  return (Object.keys(currentRecords) as (keyof PlayerRecords)[]).flatMap((key) => {
    ...
    return improved ? [{ key, label: recordLabels[key], value, ...(key === "lowestMissPercent" ? { percent: true } : {}) }] : [];
  });
}
...
export function trendForPlayer(matches: readonly Match[], playerId: PlayerId, metric: TrendMetric, mode: StatisticsMode = "all", period: StatisticsPeriod = "all"): readonly TrendPoint[] {
  return completedMatches(matches, playerId, mode, period).map((match) => {
...
export function headToHead(matches: readonly Match[], playerA: PlayerId, playerB: PlayerId, mode: StatisticsMode = "all"): HeadToHeadStatistics {
  const shared = matches.filter((match) => match.status === "completed" && match.players.includes(playerA) && match.players.includes(playerB) && (mode === "all" || match.state.kind === mode));
