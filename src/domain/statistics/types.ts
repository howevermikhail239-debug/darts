...
export type PlayerStatistics = Readonly<{
  ...
  positionScores: readonly [number, number, number];
  positions: readonly [DartPositionStatistics, DartPositionStatistics, DartPositionStatistics];
  thresholds: Readonly<Record<ThresholdKey, number>>;
  distribution: Readonly<Record<DistributionKey, number>>;
  resultSpread: number;
  winningHits: Readonly<Record<string, number>>;
  finishes: readonly number[];
}>;

export type PlayerHistoryStatistics = PlayerStatistics &
  Readonly<{
...
export type PlayerRecords = Readonly<{
  ...
  mostTriples: number;
  mostBulls: number;
  lowestMissPercent?: number;
}>;
export type RecordImprovement = Readonly<{
  key: keyof PlayerRecords;
  label: string;
  value: number;
  previous: number;
  percent?: boolean;
...
export type HeadToHeadStatistics = Readonly<{
