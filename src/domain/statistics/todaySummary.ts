...
export function summarizeToday(
  ...
  const completed = matches.filter(
    (match) => match.status === 'completed' && localDay(new Date(match.completedAt ?? match.createdAt)) === today,
  );
  ...
  for (const match of completed) {
    if (match.winnerId && persistent.has(match.winnerId)) wins.set(match.winnerId, (wins.get(match.winnerId) ?? 0) + 1);
  ...
  return {
    ...
    ...(leaderId && leaderMatch
      ? { leader: { name: leaderMatch.participantNames[leaderId] ?? 'Игрок', wins: top, tied: leaders.length > 1 } }
      : {}),
    ...(bestVisit ? { bestVisit } : {}),
    maximums,
  };
...
export function recentForm(
  ...
  limit = 5,
  statsResetAt?: string,
): readonly RecentResult[] {
  return matches
    .filter(
      (match) =>
        match.status === 'completed' &&
        match.players.includes(playerId) &&
        (!statsResetAt || match.createdAt >= statsResetAt),
    )
...
export function currentStreak(
  ...
  return {
    result,
    count: form.findIndex((item) => item !== result) < 0 ? form.length : form.findIndex((item) => item !== result),
  };
}
