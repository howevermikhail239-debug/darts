...
export function prepareResultShare(match: Match, players: readonly Player[]): ResultShareCard {
  ...
  const best = match.players.reduce((current, id) => (stats[id]?.bestVisit ?? 0) > current.score ? { score: stats[id]?.bestVisit ?? 0, name: nameOf(id) } : current, { score: 0, name: "—" });
  ...
  return {
    brand: "Dart Scorekeeper",
    date: new Date(match.completedAt ?? match.createdAt).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" }),
