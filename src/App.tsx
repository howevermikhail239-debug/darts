...
// CSS is intentionally ordered: later component layers refine the earlier foundation.
import './presentation/01-tokens-and-base.css';
import './presentation/02-concept.css';
import './presentation/03-dialogs.css';
import './presentation/04-statistics.css';
import './presentation/05-input-modes.css';
import './presentation/06-layout.css';
import './presentation/07-refinements.css';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    void countLocalData()
      .then((counts) => {
        if (alive) setWipeCounts(counts);
      })
      .catch(() => {
        if (alive) setWipeCounts({ exact: false });
      });
    return () => {
      alive = false;
    };
  ...
  const resumeView = useMemo(
    () =>
      resumeSnapshot
        ? toGameViewModel(resumeSnapshot, playersForMatches(data.players, [resumeSnapshot.match]))
        : undefined,
  ...
  const saveRawDump = useCallback(async () => {
    ...
    try {
      ...
      if (dump.unreadable) {
        setDumpNote('Локальная база не открывается — выгружать нечего.');
        return;
      }
  ...
  const updateBanner =
    appUpdate.available && !updateDismissed ? (
      <UpdateBanner
        matchInProgress={Boolean(matchInProgress)}
        onUpdate={appUpdate.apply}
      ...
      <>
        ...
        <Suspense fallback={screenFallback}>
          <HistoryPage
            ...
            players={playersForMatches(data.players, data.history)}
            persistentPlayerIds={persistentIds}
            skipped={data.shared ? 0 : match.historySkipped}
            onBack={showHome}
            onDelete={data.deleteMatch}
            onRematch={async (historicalMatch) => {
              try {
                await match.rematch(historicalMatch, data.players);
              } catch (cause) {
                setActionError(userMessage(cause, 'Не удалось начать новый матч.'));
                showHome();
              }
            }}
          />
      ...
      <>
        ...
        <Suspense fallback={screenFallback}>
          <StatisticsPage
            ...
            initialPlayerIds={statisticsContext}
            onBack={showHome}
          />
        </Suspense>
      </>
  ...
  if (screen === 'settings') {
    return (
      <>
        ...
        <SettingsPage
          players={data.players}
          onRename={data.renamePlayer}
          onResetStatistics={data.resetPlayerStatistics}
          onDeletePlayer={deleteProfile}
          onBack={showHome}
          onExport={services.exportBackup}
          onRestore={services.restoreBackup}
    ...
    <>
      ...
      {company.error ? (
        <div className="notice company-error" role="status">
          ...
          <button className="link-button" onClick={() => void company.retry().catch(() => undefined)}>
            Повторить
          </button>
        </div>
      ) : null}
      {actionError ? (
        <div className="error" role="alert">
          ...
          <button className="link-button" onClick={() => setActionError(undefined)}>
            Понятно
          </button>
        </div>
      ) : null}
      {storageNotice ? (
        <div className="notice storage-notice" role="status">
          ...
          <button className="link-button" onClick={() => setStorageNotice(undefined)}>
            Понятно
          </button>
        </div>
      ) : null}
      {match.activeIssue === 'future_version' ? (
        <div className="notice active-match-issue" role="status">
          Незавершённый матч создан более новой версией приложения и здесь не открыт.
        </div>
      ) : null}
      {match.resume && resumeView ? (
        <aside className="resume">
          <div>
            ...
            <span>
              {resumeView.title} ·{' '}
              {resumeView.match.players.map((id) => resumeView.match.participantNames[id] ?? 'Игрок').join(' / ')}
            </span>
            {resumeView.completed ? (
              <span>{resumeView.summaryTitle} · итоги сохранены</span>
            ) : (
              <span>
                Ход: {resumeView.currentPlayerName}
                {resumeView.currentPlayerDetail ? ` · ${resumeView.currentPlayerDetail}` : ''}
              </span>
            )}
            {!resumeView.completed && resumeView.draftDescription ? <span>{resumeView.draftDescription}</span> : null}
          </div>
          <button className="primary" onClick={match.continueResume}>
            {resumeView.completed ? 'Открыть итоги' : 'Продолжить'}
          </button>
          {/* DATA-3: у завершённого матча прерывать нечего — на месте разрушительной кнопки безопасное действие. */}
          {resumeView.completed ? (
            <button className="secondary" onClick={match.continueResume}>
              Показать результат
            </button>
          ) : (
            <button className="danger-quiet" onClick={() => setConfirmResumeAbandon(true)}>
              Прервать матч
            </button>
          )}
        </aside>
      ...
      <KnownCompanies
        companies={company.knownCompanies}
        currentToken={company.company?.token}
        onOpen={company.openCompany}
      />
      <SetupPage
      ...
      {match.pendingStart && match.resume ? (
        <div className="dialog-backdrop">
          <section className="active-dialog" role="dialog" aria-modal="true" aria-labelledby="active-match-title">
            ...
            <button className="primary" onClick={match.continuePending}>
              Продолжить текущую
            </button>
            <button
              className="secondary"
              onClick={() =>
                void match
                  .replaceCurrent()
                  .catch((cause: unknown) => setActionError(userMessage(cause, 'Не удалось сохранить текущий матч.')))
              }
            >
              Покинуть текущую и начать новую
            </button>
            <button className="secondary" onClick={match.cancelPending}>
              Отмена
            </button>
          </section>
        </div>
      ...
      <Dialog
        open={confirmResumeAbandon}
        title="Прервать текущий матч?"
        description="Незавершённый подход не попадёт в историю. Подтверждённые подходы сохранятся как прерванный матч."
        confirmLabel="Прервать матч"
        destructive
        onCancel={() => setConfirmResumeAbandon(false)}
        onConfirm={() => {
          setConfirmResumeAbandon(false);
          void match
            .abandonResume()
            .catch((cause: unknown) => setActionError(userMessage(cause, 'Не удалось прервать матч.')));
        }}
      />
      <Dialog
        open={confirmWipe}
        title="Стереть все данные на этом устройстве?"
        description={wipeDescriptionFor(wipeCounts)}
          ...
          void services
            ...
            .catch((cause: unknown) =>
              setWipeError(
                userMessage(
                  cause,
                  'Не удалось стереть данные. Закройте другие вкладки и окна приложения и попробуйте ещё раз.',
                ),
              ),
            );
        }}
