...
type Props={snapshot:SessionSnapshot;hint:string;canConfirm:boolean;selected:number|undefined;onSelect:(index:number)=>void;onRemove:()=>void;onReset:()=>void;onConfirm:()=>void};
export function DraftPanel({snapshot,hint,canConfirm,selected,onSelect,onRemove,onReset,onConfirm}:Props){const {draft,evaluation,isConfirming}=snapshot;const ready=evaluation.status!=='in_progress'&&evaluation.status!=='invalid';return <section className="draft-panel">
