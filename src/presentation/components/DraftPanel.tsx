...
export function DraftPanel({snapshot,selected,onSelect,onRemove,onReset,onConfirm}:Props){const {draft,evaluation,isConfirming}=snapshot;const ready=evaluation.status!=='in_progress'&&evaluation.status!=='invalid';return <section className="draft-panel">
  ...
  {!ready?<p className="hint">{snapshot.match.state.kind==='fixed_visits'?'Введите все три физических дротика':'Можно подтвердить после трёх дротиков или досрочного завершения'}</p>:null}
  </section>}
