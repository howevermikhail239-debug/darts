...
import type { ActiveMatchRecord, MatchRepository, PlayerRepository } from '../../application/ports/repositories';
...
function restoreSession(record: ActiveMatchRecord, dependencies: Dependencies): ActiveSession {
...
export function useMatchSession({ dependencies, companyToken, onShowGame, onShowHome, onSessionClosed }: Options) {
  ...
  const [activeIssue, setActiveIssue] = useState<'future_version'>();
  const [historySkipped, setHistorySkipped] = useState(0);

  useEffect(() => {
    let activeEffect = true;
    const historyRequest = dependencies.matches.readHistory
      ? dependencies.matches.readHistory()
      : dependencies.matches.listHistory().then((matches) => ({ matches, skipped: 0 }));
    void Promise.all([dependencies.players.list(), historyRequest, dependencies.matches.loadActive()])
      .then(([savedPlayers, savedHistory, savedActive]) => {
        if (!activeEffect) return;
        setPlayers(savedPlayers);
        setHistory(savedHistory.matches);
        setHistorySkipped(savedHistory.skipped);
        setActiveIssue(dependencies.matches.activeMatchIssue?.());
        if (savedActive?.current.status === 'in_progress' || savedActive?.current.status === 'completed') {
          setResume(restoreSession(savedActive, dependencies));
        }
      })
      .catch((cause: unknown) => {
  ...
  useEffect(
    () =>
      dependencies.matches.onExternalChange?.(() => {
        void Promise.all([
          ...
          .then(([savedActive, savedHistory]) => {
            ...
            const restored = restoreSession(savedActive, dependencies);
            setActive((current) => (current ? restored : undefined));
            setResume((current) => (current ? restored : undefined));
          })
          .catch((cause: unknown) => {
            console.error('Не удалось применить изменение из другой вкладки:', cause);
            setError('Матч изменён в другой вкладке. Обновите страницу.');
          });
      }),
    [dependencies, onShowHome],
  );

  const performStart = useCallback(
    async (participants: readonly SetupParticipant[], setup: MatchSetup) => {
  ...
  const renamePlayer = useCallback(
    async (playerId: string, name: string) => {
      ...
      if (!trimmed || trimmed.length > 80) throw new Error('Имя должно содержать от 1 до 80 символов.');
      const player = (await dependencies.players.list()).find((item) => item.id === playerId);
      if (!player) throw new Error('Игрок не найден.');
      await dependencies.players.save({ ...player, name: trimmed });
      setPlayers(await dependencies.players.list());
    },
    [dependencies],
  );
  const resetPlayerStatistics = useCallback(
    async (playerId: string) => {
      const player = (await dependencies.players.list()).find((item) => item.id === playerId);
      if (!player) throw new Error('Игрок не найден.');
      await dependencies.players.save({ ...player, statsResetAt: dependencies.now() });
      setPlayers(await dependencies.players.list());
    },
    [dependencies],
  );
  ...
  const deletePlayer = useCallback(
    async (playerId: string) => {
      ...
      await dependencies.players.delete(playerId);
      setPlayers(await dependencies.players.list());
    },
    [dependencies],
  );
  const deleteMatch = useCallback(
    async (matchId: string) => {
      if (!dependencies.matches.deleteHistory) throw new Error('Удаление матча недоступно.');
      await dependencies.matches.deleteHistory(matchId);
  ...
  const rematch = useCallback(
    async (source: Match, participantCatalog: readonly Player[] = players) => {
