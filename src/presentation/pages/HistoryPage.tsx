...
export function HistoryPage({ matches, players, persistentPlayerIds, onBack, onRematch }: { matches: readonly Match[]; players: readonly Player[]; persistentPlayerIds: readonly PlayerId[]; onBack: () => void; onRematch: (match: Match) => Promise<void> }) {
  const groups = matches.reduce<Array<{ key: string; label: string; matches: Match[] }>>((result, match) => {
    const key = dayKey(match.createdAt), existing = result.find((group) => group.key === key);
    if (existing) existing.matches.push(match); else result.push({ key, label: dayLabel(match.createdAt), matches: [match] });
    return result;
  }, []);
  return <main className="history-page">
    ...
    {matches.length === 0 ? <p className="empty">{t.empty}</p> : groups.map((group) => <section className="history-day" key={group.key}><h2>{group.label}</h2>{group.matches.map((match) => {
