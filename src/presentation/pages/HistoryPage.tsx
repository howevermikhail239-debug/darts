...
export function HistoryPage({
  ...
  const groups = useMemo(() => {
    const result: Array<{ key: string; label: string; matches: Match[] }> = [],
      byDay = new Map<string, Match[]>();
    for (const match of visible) {
      const key = dayKey(match.createdAt),
        existing = byDay.get(key);
      if (existing) existing.push(match);
      else {
        const groupMatches = [match];
        byDay.set(key, groupMatches);
        result.push({ key, label: dayLabel(match.createdAt), matches: groupMatches });
      }
    }
    return result;
  }, [visible]);
  ...
  return (
    <main className="history-page">
      ...
      {skipped > 0 ? (
        <p className="notice history-skipped" role="status">
          Пропущено повреждённых записей: {skipped}.
        </p>
      ) : null}
      {matches.length === 0 ? (
        <p className="empty">Здесь появятся сыгранные матчи.</p>
      ) : (
        groups.map((group) => (
          <section className="history-day" key={group.key}>
            ...
            {group.matches.map((match) => (
              <HistoryMatch
                ...
                match={match}
                players={players}
                persistentPlayerIds={persistentPlayerIds}
                onRematch={onRematch}
                onDelete={onDelete}
              />
            ))}
      ...
      {remaining > 0 ? (
        <div className="history-more">
          <button className="secondary" onClick={() => setVisibleCount((current) => current + PAGE_SIZE)}>
            Показать ещё
          </button>
          <small>
            Показано {visible.length} из {matches.length}
          </small>
        </div>
      ) : null}
    </main>
...
function HistoryMatch({
  ...
  const mode =
    match.state.kind === 'x01'
      ? `X01 · ${match.state.startingScore}`
      : `Серия · ${match.state.visitsPerPlayer} подходов`;
  const result = match.players
    .map((id) => (match.state.kind === 'x01' ? match.state.remaining[id] : match.state.totals[id]))
    .join(' : ');
  return (
    <details className="history-match" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
      <summary>
        <div>
          <b>{playerNames.join(' — ')}</b>
          <span>
            {mode} · {new Date(match.createdAt).toLocaleDateString('ru-RU')}
          </span>
          <span className={match.status === 'abandoned' ? 'history-status abandoned' : 'history-status'}>
            {match.status === 'abandoned' ? 'Матч прерван' : winner ? `Победитель: ${winner}` : 'Ничья'}
          </span>
        </div>
      ...
      {open ? (
        <HistoryDetail
          ...
          players={players}
          persistentPlayerIds={persistentPlayerIds}
          onRematch={async (source) => {
            setError(undefined);
            try {
              await onRematch(source);
            } catch (cause) {
              setError(userMessage(cause, 'Не удалось начать новый матч.'));
            }
          }}
      ...
      <Dialog
        ...
        description="Матч исчезнет из истории, а вся зависимая статистика будет пересчитана."
        ...
        onConfirm={() => {
          ...
          void onDelete(match.id)
            .catch((cause: unknown) => setError(userMessage(cause, 'Не удалось удалить матч.')))
  ...
  return (
    <div className="history-detail">
      ...
      {match.players.map((id, index) => {
        const current = currentNameOf(match, players, id);
        return (
          <div className="history-player" key={id}>
            <PlayerIdentity
              playerId={id}
              name={nameOf(match, id)}
              temporary={!persistentPlayerIds.includes(id)}
              position={index}
              compact
            />
            {current ? <small className="history-renamed">сейчас {current}</small> : null}
            <span>
              среднее {stats[id]?.averagePerVisit.toFixed(1)} · лучший {stats[id]?.bestVisit} · дротиков{' '}
              {stats[id]?.physicalDarts}
            </span>
          </div>
        );
      })}
      <ol>
        {match.confirmedVisits.map((visit) => (
          <li key={visit.id}>
            {nameOf(match, visit.playerId)}:{' '}
            {visit.inputKind === 'aggregate'
              ? `Сумма подхода: ${visit.aggregateScore}`
              : visit.darts.map(notationOf).join(' · ')}{' '}
            = {visit.rawScore}
            {visit.result === 'bust' ? ' (перебор)' : ''}
          </li>
        ))}
      </ol>
