...
export function SettingsPage({ players, onRename, onResetStatistics, onDeletePlayer, onBack, onExport, onRestore, hapticsSupported, hapticsEnabled, onHaptics }: Props) {
  ...
  const forgetEdit = (playerId: string) => setEdits((current) => {
  ...
  const download = async () => {
    ...
    } catch (cause) { setMessage(userMessage(cause, "Не удалось экспортировать данные.")); }
  ...
  const restore = async (selected: File) => {
    ...
    catch (cause) { setMessage(userMessage(cause, "Не удалось восстановить данные.")); }
  ...
  return <main className="settings-page">
    ...
    <section className="setup-form player-management"><h2>Игроки</h2>{players.length === 0 ? <p>Сохранённых игроков пока нет.</p> : players.map((player) => <article key={player.id}><label>Имя<input maxLength={80} value={nameFieldOf(player)} onChange={(event) => setEdits((current) => ({ ...current, [player.id]: event.target.value }))} /></label><div><button className="secondary" disabled={busy || !nameFieldOf(player).trim() || nameFieldOf(player).trim() === player.name} onClick={() => { setBusy(true); setMessage(undefined); void onRename(player.id, nameFieldOf(player)).then(() => { forgetEdit(player.id); setMessage('Имя игрока сохранено.'); }).catch((cause: unknown) => setMessage(userMessage(cause, "Не удалось переименовать игрока."))).finally(() => setBusy(false)); }}>Переименовать</button><button className="secondary" disabled={busy} onClick={() => setPending({ kind: 'reset', player })}>Обнулить статистику</button><button className="danger-button" disabled={busy} onClick={() => setPending({ kind: 'delete', player })}>Удалить профиль</button></div>{player.statsResetAt ? <small>Статистика считается заново с {new Date(player.statsResetAt).toLocaleString('ru-RU')}</small> : null}</article>)}</section>
