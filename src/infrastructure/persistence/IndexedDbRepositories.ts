...
export const BLOCKING_MESSAGE =
  'Приложение открыто в другой вкладке и обновляет локальную базу. Обновите эту страницу.';
...
async function migrateMetaRows(transaction: UpgradeTransaction): Promise<void> {
  ...
  for (const rawKey of await meta.getAllKeys()) {
    ...
    if (key.startsWith('company:')) {
      ...
      const value = await meta.get(rawKey);
      if (isString(token) && Array.isArray(value)) await companyPlayers.put({ token, players: value.filter(isPlayer) });
      await meta.delete(rawKey);
...
function upgrade(
  ...
  if (!database.objectStoreNames.contains('companyPlayers'))
    database.createObjectStore('companyPlayers', { keyPath: 'token' });
...
const db = (): Promise<IDBPDatabase<DartsDb>> => {
  if (!database) {
    database = openDB<DartsDb>(DB_NAME, DB_VERSION, {
      upgrade,
      blocked() {
        notify('blocked', BLOCKED_MESSAGE);
      },
      blocking() {
...
function isLastSetup(value: unknown): value is LastSetupTemplate {
  ...
  if (
    ...
    value.participants.length > 8 ||
    !value.participants.every(
      (participant) =>
        isRecord(participant) &&
        isString(participant.name) &&
        (participant.playerId === undefined || isString(participant.playerId)),
    )
  )
    return false;
  if (
    !isInteger(value.setup.startingPlayerIndex) ||
    value.setup.startingPlayerIndex < 0 ||
    value.setup.startingPlayerIndex >= value.participants.length
  )
    return false;
  if (value.setup.mode === 'fixed_visits')
    return (
      isInteger(value.setup.visitsPerPlayer) && value.setup.visitsPerPlayer >= 1 && value.setup.visitsPerPlayer <= 999
    );
  if (
    ...
    return false;
  return (
    value.setup.format.kind === 'unlimited' ||
    (value.setup.format.kind === 'limited' &&
      isInteger(value.setup.format.visitsPerPlayer) &&
      value.setup.format.visitsPerPlayer >= 1 &&
      value.setup.format.visitsPerPlayer <= 999)
  );
}
function isDraft(value: unknown): value is VisitDraft {
  ...
  if (value.kind === 'aggregate')
    return (
      Array.isArray(value.darts) &&
      value.darts.length === 0 &&
      (value.score === undefined || (typeof value.score === 'number' && isReachableThreeDartScore(value.score)))
    );
  return (
    (value.kind === undefined || value.kind === 'detailed') &&
    Array.isArray(value.darts) &&
    value.darts.length <= 3 &&
    value.darts.every(isDart)
  );
}
function isActiveDraft(value: unknown, match: Match): value is ActiveVisitDraft {
  return (
    ...
    value.playerId === match.players[match.currentPlayerIndex] &&
    isDraft(value.draft)
  );
}
...
function migrateActive(value: unknown): unknown {
  ...
  return {
    ...value,
    current: migrateMatch(value.current),
    ...(value.previous !== undefined ? { previous: migrateMatch(value.previous) } : {}),
  };
}
...
export class IndexedDbMatchRepository implements MatchRepository {
  ...
  async loadActive(): Promise<ActiveMatchRecord | undefined> {
    ...
    if (!isActiveMatchEnvelope(value)) throw new Error('Сохранённый матч повреждён. Сбросьте локальные данные.');
    const base =
      value.previous === undefined ? { current: value.current } : { current: value.current, previous: value.previous };
    ...
    if ((value.schemaVersion === 2 || value.schemaVersion === 3) && isActiveDraft(value.draft, value.current))
      return {
        ...
        draft: value.draft,
        revision,
        ...(isString(value.companyToken) ? { companyToken: value.companyToken } : {}),
      };
    return {
      ...base,
  ...
  async archiveAndClearActive(match: Match): Promise<void> {
    await this.serialize(async () => {
      ...
      if (active?.companyToken && match.status !== 'in_progress')
        await transaction.objectStore('companyMatches').put(
          structuredClone({
            token: active.companyToken,
            matchId: match.id,
            match,
...
export class IndexedDbPlayerRepository implements PlayerRepository {
  async list(): Promise<readonly Player[]> {
    return (await (await db()).getAll('players')).sort((a, b) => a.name.localeCompare(b.name, 'ru'));
  }
...
export class IndexedDbSharedRepository {
  async companies(): Promise<readonly SharedCompany[]> {
    return (await (await db()).getAll('companies')).filter(
      (value): value is SharedCompany =>
        isRecord(value) && isString(value.token) && typeof value.name === 'string' && isString(value.createdAt),
    );
  }
  ...
  async updatePlayers(
    token: string,
    mutate: (current: readonly Player[]) => readonly Player[],
  ...
  async pendingMatches(token: string): Promise<readonly SharedMatchCache[]> {
    ...
    const rows = (
      await Promise.all(
        (['pending', 'error'] as const).map((state) =>
          connected.getAllFromIndex('companyMatches', 'byTokenState', IDBKeyRange.only([token, state])),
        ),
      )
    ).flat();
    return rows.map(toCache).filter((row): row is SharedMatchCache => row !== undefined);
  ...
  async mergeRemote(token: string, matches: readonly Match[]): Promise<void> {
    ...
    const deletedIds = new Set(
      (await tombstones.getAll(IDBKeyRange.bound([token, ''], [token, '￿']))).map((row) => row.matchId),
    );
...
export class LocalSettingsRepository implements SettingsRepository {
  async load(): Promise<Readonly<Record<string, string>>> {
    ...
    try {
      const legacy: unknown = JSON.parse(localStorage.getItem('darts-settings-v1') ?? '{}');
      return isSettings(legacy) ? legacy : {};
    } catch {
      return {};
    }
  }
...
export class IndexedDbBackupRepository implements BackupRepository {
  async readAll(): Promise<BackupData> {
    ...
    if (!players.every(isPlayer) || !validMatches.every(isMatch) || !isSettings(settings ?? {}))
      throw new Error('Локальные данные повреждены.');
    const activeRecord =
      migratedActive === undefined
        ? undefined
        : (() => {
    ...
    return {
      ...
      companies: companies.filter(
        (company): company is SharedCompany =>
          isRecord(company) &&
          isString(company.token) &&
          typeof company.name === 'string' &&
          isString(company.createdAt),
      ),
  ...
  async replaceAll(data: BackupData): Promise<void> {
    ...
    if (!matches.every(isMatch) || !isSettings(data.settings))
      throw new Error('Резервная копия содержит некорректные данные.');
    ...
    if (data.active !== undefined) {
      ...
      if (!isActiveMatchEnvelope(candidate) || !isActiveDraft(candidate.draft, candidate.current))
        throw new Error('Активный матч в копии повреждён.');
      activeEnvelope = candidate;
    }
    const companies = (data.companies ?? []).filter(
      (company) =>
        isRecord(company) && isString(company.token) && typeof company.name === 'string' && isString(company.createdAt),
    );
    const companyPlayers = (data.companyPlayers ?? []).filter(
      (row) => isRecord(row) && isString(row.token) && Array.isArray(row.players),
    );
    const companyMatches = (data.companyMatches ?? [])
      .map((row) =>
        isRecord(row) && isString(row.token) && isString(row.matchId)
          ? { row, match: migrateMatch(row.match) }
          : undefined,
      )
    ...
    for (const company of companies)
      await transaction
        ...
        .put(structuredClone({ token: company.token, name: company.name, createdAt: company.createdAt }));
    for (const row of companyPlayers)
      await transaction
        .objectStore('companyPlayers')
        .put(structuredClone({ token: row.token, players: row.players.filter(isPlayer) }));
    for (const { row, match } of companyMatches)
      await transaction.objectStore('companyMatches').put(
        structuredClone({
          token: row.token,
          matchId: row.matchId,
